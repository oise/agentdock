  ...
  return (
    <div
      ...
      className={`text-foreground group relative pl-1 pr-2 flex h-full max-w-[180px] shrink items-center rounded-[4px] 
        ...
        ${
          isActive
            ? 'text-foreground before:absolute before:inset-0 before:bg-background before:[filter:var(--ide-surface-active-filter)] ' +
    ...
    >
      ...
      >
        <div className='flex shrink-0 items-center relative left-[1px] opacity-80'>{getTabIcon(tab, agents)}</div>
        <div className={`min-w-0 flex-1 overflow-hidden ${titleClassName}`}>
          <Tooltip variant='minimal' content={tab.title} className='min-w-0' position='bottom' delay={300}>
            <div className={`truncate text-ide-small relative top-[1px] ${hasProcessing ? 'tab-shimmer-text' : ''}`}>
              {tab.title}
            </div>
          </Tooltip>
        </div>
